...
from .extraction import extract_html_document, extract_pdf_document
from .models import AcquiredPaper, PaperDocument, PaperSummaryError
...
class ArxivSourceClient:
    ...
    def _result(
        ...
        path = source_directory(arxiv_id) / f"source.{suffix}"
        _atomic_write(path, raw)
        return AcquiredPaper(arxiv_id, kind, path, _sha256(raw), document)
    ...
    def acquire(self, arxiv_id: str, expected_title: str) -> AcquiredPaper:
        ...
        if html_status == 200 and html_type in {"text/html", "application/xhtml+xml"}:
            try:
                ...
                return self._result(paper_id, "html", html_raw, document)
        ...
        return self._result(paper_id, "pdf", pdf_raw, document)
