...
def build_parser() -> argparse.ArgumentParser:
    ...
    run.add_argument("--model", default=os.environ.get("TOGOS_MODEL"), help="local model name")
    run.add_argument(
        "--base-url",
        default=os.environ.get("TOGOS_MODEL_BASE_URL", DEFAULT_BASE_URL),
...
def _execute(argv: list[str] | None = None) -> int:
    ...
    if not isinstance(arguments.model, str) or not arguments.model.strip():
        raise PaperSummaryError("model_required", "pass --model or set TOGOS_MODEL")
    ...
    for record in result.records:
        if record.status == "failed":
            print(
                f"failed {record.arxiv_id} {record.error_code}: {record.error_message}",
                file=sys.stderr,
            )
